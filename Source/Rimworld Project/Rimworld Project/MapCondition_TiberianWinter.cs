...
{
    public class MapCondition_TiberianWinter : MapCondition
    {
        private int LerpTicks = 5000;
        ...
        public MapCondition_TiberianWinter()
        {
            ColorInt colorInt = new ColorInt(0, 239, 208);
        ...
        public override void MapConditionTick()
        {
            ...
            {
                List<Pawn> allPawnSpawned = base.Map.mapPawns.AllPawnsSpawned;
                for (int i=0; i < allPawnSpawned.Count; i++)
                {
                    Pawn pawn = allPawnSpawned[i];
                    if (!pawn.Position.Roofed(base.Map) && pawn.def.race.IsFlesh)
                    {
                        ...
                        if (num != 0f)
                        {
                            ...
                            Rand.PopSeed();
                            num *= num2;
                            HealthUtility.AdjustSeverity(pawn, HediffDefOf_Tiberium.TiberiumBuildup, num);
                        }
                    }
                }
            }
